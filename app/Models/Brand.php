...
class Brand // extends Model
{
    ...
    function save($arr,$id=null,$ss=null){
        ...
        $inputs = $res->result;
        $exist = isExist('brands',$id,['name' =>$inputs['name']]);
        if($exist) return ObjectRes::error('Brand name is already taken');
        $newID = saveData('brands',['id'=>$id],$inputs,$ss);
    ...
    function listPaginate($arr,$ss=null){
        ...
        $selectCols = 'b.name,b.id';
        $query = DB::table('brands as b')->selectRaw($selectCols);
        $count_clone = clone($query);
        $count = $count_clone->count('b.id');
        $rows = $query->skip($skip_rows)->take($per_page)->orderBy('b.id','desc')->get();
    ...
    function details($id=null,$ss=null){
        $ss = $ss?$ss:$this->ss;
        $id = $id?$id:$this->id;
        $row = DB::table('brands as b')->where('b.id',$id)->selectRaw('b.name,b.id')->first();
        return $row;
    }
